...
class Pages:
    ...
    def __init__(self, ctx, *, entries, per_page=12, show_entry_count=True):
        ...
        self.embed = discord.Embed(
            colour=0x525a32)  # any HEX color here
            ...
            try:
                tasks = [
                    asyncio.ensure_future(self.bot.wait_for(
                        'raw_reaction_add', check=self.react_check)),
                    asyncio.ensure_future(self.bot.wait_for(
                        'raw_reaction_remove', check=self.react_check))
                ]
...
class HelpPaginator(Pages):
    ...
    async def show_page(self, page, *, first=False):
        ...
        if hasattr(self, '_is_bot'):
            value = 'For more help, join the official bot [support server](https://discord.gg/nwUjNWSG)'
        ...
        for entry in entries:
            self.embed.add_field(name=signature(
                entry), value=entry.description or "No help provided for now try it yourself", inline=False)
